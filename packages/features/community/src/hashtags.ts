...
const HASHTAG_PATTERN =
  /(^|[^\p{L}\p{N}_])#([\p{L}\p{N}][\p{L}\p{N}_-]{0,39})/gu;
...
export function buildTrendingHashtags(
  ...
  for (const dua of duas) {
    for (const hashtag of extractHashtags(dua.text)) {
      const current = counts.get(hashtag.tag) ?? {
        ...hashtag,
        duas: 0,
        ameens: 0,
      };
  ...
  return [...counts.values()]
    .sort(
      (a, b) =>
        b.duas - a.duas || b.ameens - a.ameens || a.tag.localeCompare(b.tag),
    )
    .slice(0, limit);
}

export type TrendingByLanguage = Record<
  'all' | 'en' | 'ar' | 'es' | 'ur' | 'fr',
  TrendingHashtag[]
>;
