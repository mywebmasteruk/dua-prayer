...
describe("AI dua moderation", () => {
  it("keeps hardship prayer language safe without AI", async () => {
    const result = await evaluateDuaModeration({
      ...
      settings: { enabled: false, provider: "none", model: "", apiKey: null, modelMode: "manual", moderationTimeoutMs: 9000, reasoningEffort: "low", maxTokens: 1200, temperature: 0.4, requestTimeoutMs: 20000 },
    })
  ...
  it("blocks severe abusive local content before provider calls", async () => {
    ...
    const result = await evaluateDuaModeration({
      ...
      settings: { enabled: true, provider: "openai", model: "gpt-4o-mini", apiKey: "sk-test", modelMode: "manual", moderationTimeoutMs: 9000, reasoningEffort: "low", maxTokens: 1200, temperature: 0.4, requestTimeoutMs: 20000 },
