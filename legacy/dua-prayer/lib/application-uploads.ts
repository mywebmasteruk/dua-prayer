...
import type { FormKind } from "@/lib/form-fields"
...
function sanitizeFilename(name: string): string {
  const trimmed = name.trim().toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/-+/g, "-")
...
export async function createSignedUploadUrl(request: SignedUploadRequest): Promise<SignedUploadResult> {
  ...
  const { data, error } = await admin.storage.from(APPLICATION_UPLOADS_BUCKET).createSignedUploadUrl(path)
  ...
  return { ok: true, bucket: APPLICATION_UPLOADS_BUCKET, path: data.path, token: data.token }
}
