const arabicCharacterPattern =
  /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/g;
export const arabicTextSegmentPattern =
  /([\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]+)/g;
...
export function getArabicOnlyFontClassName(text: string) {
  return hasArabicText(text) && !latinCharacterPattern.test(text)
    ? arabicFontClassName
    : '';
}

export function isLatinScriptLanguage(
  language: string | null | undefined,
  text = '',
) {
  const normalizedLanguage = language?.trim().toLowerCase();

  if (normalizedLanguage && latinScriptLanguages.has(normalizedLanguage)) {
    return true;
  }

  if (normalizedLanguage && nonLatinScriptLanguages.has(normalizedLanguage)) {
    return false;
  }

  return (
    latinScriptLetterPattern.test(text) && !nonLatinLetterPattern.test(text)
  );
}
