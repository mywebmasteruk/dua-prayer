export const DUA_SHARE_URL = 'https://www.duaprayer.com';
...
export function buildDuaSharePayload(dua: ShareableDua): DuaSharePayload {
  ...
  const availableDuaLength = Math.max(
    0,
    MAX_SHARE_TEXT_LENGTH - SHARE_SUFFIX.length - 2,
  );
  ...
  return {
    title: 'DuaPrayer',
    text,
    url: `${DUA_SHARE_URL}/#dua-${dua.id}`,
  };
}

export function buildDuaShareUrl(
  platform: DuaSharePlatform,
  dua: ShareableDua,
) {
